...
param(
    ...
    [ValidateSet("sketch2bim", "kushalsamant-github-io", "all")]
    [string]$Project = "all",
...
# Project configurations - Updated for unified platform
# All apps are now deployed from root directory with subdomain routing
$projects = @(
    @{
        Name = "kushalsamant-github-io"
        DisplayName = "Unified Platform (Sketch2BIM)"
        RootDirectory = $null  # Root directory for unified deployment
        EnvFile = ".env.local.frontend"  # Reference file (not used for sync, use Vercel dashboard for frontend vars)
        Prefix = ""
        FrontendVars = @(
            # Platform-wide variables
            "NEXT_PUBLIC_PLATFORM_API_URL", "NEXT_PUBLIC_AUTH_URL",
            # Sketch2BIM variables
