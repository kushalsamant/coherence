            ...
            {Object.entries(unitEcon).map(([project, metrics]) => (
              <tr key={project} className="border-b">
                ...
                <td className="p-2 text-right">
                  ${metrics.revenue_per_user_usd.toFixed(2)}
                </td>
                <td className="p-2 text-right">
                  ${metrics.cost_per_user_usd.toFixed(6)}
                </td>
                <td className={`p-2 text-right ${metrics.contribution_margin_usd >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                  ${metrics.contribution_margin_usd.toFixed(2)}
                </td>
                <td className={`p-2 text-right ${metrics.gross_margin_percent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
