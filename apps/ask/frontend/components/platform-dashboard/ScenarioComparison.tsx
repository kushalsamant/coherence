            ...
            <div key={key} className="border rounded-lg p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-medium ${scenario.monthly_profit_usd >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ${scenario.monthly_profit_usd.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Net Margin:</span>
                  <span className={`font-medium ${scenario.net_margin_percent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {scenario.net_margin_percent.toFixed(2)}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Annual Profit:</span>
                  <span className={`font-medium ${scenario.annual_profit_usd >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ${scenario.annual_profit_usd.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
                <div className="pt-2 border-t">
                  <span className={`text-xs font-medium ${scenario.is_profitable ? 'text-green-600' : 'text-red-600'}`}>
