...
export async function signIn() {
  ...
  const currentUrl = typeof window !== "undefined" 
    ...
    : process.env.REFRAME_FRONTEND_URL || "https://reframe.kvshvl.in";
  
  redirect(`${authUrl}/api/auth/signin?app=reframe&returnUrl=${encodeURIComponent(currentUrl)}`);
}
...
export const handlers = {
  GET: async () => {
    redirect(`${process.env.NEXT_PUBLIC_AUTH_URL || "https://kvshvl.in"}/api/auth/signin?app=reframe`);
  },
  POST: async () => {
    redirect(`${process.env.NEXT_PUBLIC_AUTH_URL || "https://kvshvl.in"}/api/auth/signin?app=reframe`);
  },
