"""
Database migration utilities for ASK
...
def get_alembic_config() -> Config:
    """
    Get Alembic configuration object for ASK database
    """
    # Get the repository root (3 levels up from utils/ask/)
    repo_root = Path(__file__).resolve().parents[4]
    alembic_ini_path = repo_root / "database" / "migrations" / "ask" / "alembic.ini"
...
def run_migrations() -> bool:
    ...
    try:
        logger.info("Running ASK database migrations...")
        alembic_cfg = get_alembic_config()
        
        # Run migrations to head
        command.upgrade(alembic_cfg, "head")
        
        logger.success("ASK database migrations completed successfully")
        return True
        
    except Exception as e:
        logger.error(
            f"Failed to run ASK database migrations: {e}",
            exc_info=True
        )
        logger.warning(
