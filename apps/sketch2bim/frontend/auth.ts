...
export async function signIn() {
  ...
  const currentUrl = typeof window !== "undefined" 
    ...
    : process.env.SKETCH2BIM_FRONTEND_URL || "https://sketch2bim.kvshvl.in";
  
  redirect(`${authUrl}/api/auth/signin?app=sketch2bim&returnUrl=${encodeURIComponent(currentUrl)}`);
}
...
export const handlers = {
  GET: async () => {
    redirect(`${process.env.NEXT_PUBLIC_AUTH_URL || "https://kvshvl.in"}/api/auth/signin?app=sketch2bim`);
  },
  POST: async () => {
    redirect(`${process.env.NEXT_PUBLIC_AUTH_URL || "https://kvshvl.in"}/api/auth/signin?app=sketch2bim`);
  },
